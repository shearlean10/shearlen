...
if run_as_module:
    ...
    file = f"sherlean{sys.argv[6]}.log" if len(sys.argv) > 6 else "sherlean.log"

    if os.path.exists(file):
        os.remove(file)

    HOSTED_ON = where_hosted()
    LOGS = getLogger("Sherlean Logs")
    ...
    LOGS.info(f"sherlibs Version - {__Ayra__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Shearlean Version - {ayra_version} [{HOSTED_ON}]")
